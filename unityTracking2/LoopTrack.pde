class LoopPoint {
  ...
  String toString(){
      String s = idx + "," + int(time) + "," + (int(speed * 10) /10.0) + "," + status + "," 
...
class LoopTrack {
  ...
  void writePoint(LoopPoint last){
    ...
    if(last.status == "start"){
      String s = "Car Id,Tramo Id,UTM Index,Car Time,Speed,Status,Utm X,Utm Y,Norm X,Norm Y,Avg Speed,Track Time,Distance,Remaining Distance,Error";
      output.println(s);
      output2.println(s);
    }
    String s = "";
